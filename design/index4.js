...
//                    } ); 
    ...
    marker = this.add.graphics();           //畫正方形
    marker.lineStyle(2, 0xdddddd, 1);
    marker.strokeRect(0, 0, 48, 48);    //map.tileWidth, map.tileHeight);

    this.input.on('pointerdown', function (pointer) {
        $('#run_over').show();//蓋住執行按鈕
...
//    {
    ...
    //            //console.log(typeof(pointerTileX));console.log(typeof(pointerTileY));
            ...
            player[0].nowx= XX;       
            player[0].nowy= YY;
            player[0].face= selectedTile - 300;         //在index 的div 按下時呼叫函數時已做編號
            
            player[0].role.x = (XX - 1) * 48;
            player[0].role.y = (YY - 1) * 48;
            player[0].role.anims.play(fff[player[0].face], true);
        }
//    }
...
function update(time, delta) {
  ...
  //if(s.x>-5){s.x=-120;} else{s.x+=0.5}          //控制背景移動
    ...
    if((worldPoint.x>0)&&(worldPoint.x<480)&&(worldPoint.y>0)&&(worldPoint.y<480)){
...
function get_object(num) {              //取得物品(物品代號)
    ...
    if(diamondNum == diamondNeed[stageNum]){
        ...
        let strWin ="<br>恭喜您檢驗通過了！ <br>累計共花費 " +(timeCount/10)+ "秒<br>將自動進到下一關！";
